...
class BinanceWS {
     ...
     */
    constructor(beautify=false, heartbeat_interval=60000, renewal_interval=30) {
        if(beautify)
            this.beautify = false;
        ...
        let until_renewal = renewal_interval;
        this.heartbeat_interval = setInterval(() => {
            ...
            if(until_renewal == 0){
                until_renewal = renewal_interval;
    ...
    _beautifyResponse(data) {
        if (data instanceof Array) {
            ...
            for(let i = 0; i < data.length; ++i)
                new_data[i] = event.e ? this._beautifier.beautify(event, event.e + 'Event') : event;
        ...
        return binanceRest.startUserDataStream()
            .then((response) => {
                ...
                ws.on('close', () => {
                    ...
                    binanceRest.closeUserDataStream(response);
                });
            });
    }
