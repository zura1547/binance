class Beautifier {
    ...
    beautify(object, type) {
        const beautifications = typeof type !== 'object' ? this._beautifications[type] : type;
        if (!beautifications) {
            return object;
        }
        let newItem = {};
        for(let key in object) {
            ...
            if (value instanceof Array) {
                const newArray = [];
                if (this._beautifications[beautifications[key]] instanceof Array) {
                    ...
                    for(let i = 0; i < value.length; ++i)
                        newArray.push(this.beautify(value[i], beautifications[i]));
